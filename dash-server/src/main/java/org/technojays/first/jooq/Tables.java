/**
 * This class is generated by jOOQ
 */
package org.technojays.first.jooq;


import javax.annotation.Generated;

import org.technojays.first.jooq.tables.Alliance;
import org.technojays.first.jooq.tables.AllianceRef;
import org.technojays.first.jooq.tables.AllianceType;
import org.technojays.first.jooq.tables.Ally;
import org.technojays.first.jooq.tables.AttributeType;
import org.technojays.first.jooq.tables.Award;
import org.technojays.first.jooq.tables.AwardType;
import org.technojays.first.jooq.tables.Competition;
import org.technojays.first.jooq.tables.Event;
import org.technojays.first.jooq.tables.Game;
import org.technojays.first.jooq.tables.Location;
import org.technojays.first.jooq.tables.Match;
import org.technojays.first.jooq.tables.MatchScore;
import org.technojays.first.jooq.tables.Program;
import org.technojays.first.jooq.tables.ScoreType;
import org.technojays.first.jooq.tables.Sponsor;
import org.technojays.first.jooq.tables.Team;
import org.technojays.first.jooq.tables.TeamAttribute;
import org.technojays.first.jooq.tables.TeamScore;
import org.technojays.first.jooq.tables.TeamSponsor;


/**
 * Convenience access to all tables in first
 */
@Generated(
	value = {
		"http://www.jooq.org",
		"jOOQ version:3.6.1"
	},
	comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class Tables {

	/**
	 * Relationship between a match and a group of allies
	 */
	public static final Alliance ALLIANCE = org.technojays.first.jooq.tables.Alliance.ALLIANCE;

	/**
	 * Alliance reference of hash generated from ordered list of teams
	 */
	public static final AllianceRef ALLIANCE_REF = org.technojays.first.jooq.tables.AllianceRef.ALLIANCE_REF;

	/**
	 * Alliance types for FIRST programs
	 */
	public static final AllianceType ALLIANCE_TYPE = org.technojays.first.jooq.tables.AllianceType.ALLIANCE_TYPE;

	/**
	 * Relationship between a set of teams for an alliance
	 */
	public static final Ally ALLY = org.technojays.first.jooq.tables.Ally.ALLY;

	/**
	 * Attribute types in program
	 */
	public static final AttributeType ATTRIBUTE_TYPE = org.technojays.first.jooq.tables.AttributeType.ATTRIBUTE_TYPE;

	/**
	 * Awards awarded at competitions and events
	 */
	public static final Award AWARD = org.technojays.first.jooq.tables.Award.AWARD;

	/**
	 * Award types
	 */
	public static final AwardType AWARD_TYPE = org.technojays.first.jooq.tables.AwardType.AWARD_TYPE;

	/**
	 * A competition describes a Location, Game, and is held at an Event
	 */
	public static final Competition COMPETITION = org.technojays.first.jooq.tables.Competition.COMPETITION;

	/**
	 * An event describes the timeframe for competitions and celebrations that are managed together

	 */
	public static final Event EVENT = org.technojays.first.jooq.tables.Event.EVENT;

	/**
	 * A game describes a specific set of rules and competitions for a program and its teams. It has a period of time 
associated with it.
	 */
	public static final Game GAME = org.technojays.first.jooq.tables.Game.GAME;

	/**
	 * Locations for program
	 */
	public static final Location LOCATION = org.technojays.first.jooq.tables.Location.LOCATION;

	/**
	 * Describe a match where teams competed against each other

	 */
	public static final Match MATCH = org.technojays.first.jooq.tables.Match.MATCH;

	/**
	 * Scores associated with a match
	 */
	public static final MatchScore MATCH_SCORE = org.technojays.first.jooq.tables.MatchScore.MATCH_SCORE;

	/**
	 * A program is a sub-organization of FIRST that describes the type of games that will be played and the 
types of participants.
	 */
	public static final Program PROGRAM = org.technojays.first.jooq.tables.Program.PROGRAM;

	/**
	 * Types of scores associated with matches
	 */
	public static final ScoreType SCORE_TYPE = org.technojays.first.jooq.tables.ScoreType.SCORE_TYPE;

	/**
	 * Sponsors for teams, events, groups, programs, etc

	 */
	public static final Sponsor SPONSOR = org.technojays.first.jooq.tables.Sponsor.SPONSOR;

	/**
	 * A team associated with a specific FIRST program
	 */
	public static final Team TEAM = org.technojays.first.jooq.tables.Team.TEAM;

	/**
	 * Attributes associated with a FIRST team
	 */
	public static final TeamAttribute TEAM_ATTRIBUTE = org.technojays.first.jooq.tables.TeamAttribute.TEAM_ATTRIBUTE;

	/**
	 * Scores associated with a FIRST team during a match at a competition

	 */
	public static final TeamScore TEAM_SCORE = org.technojays.first.jooq.tables.TeamScore.TEAM_SCORE;

	/**
	 * Sponsors associated with a FIRST team for a given game (season)
	 */
	public static final TeamSponsor TEAM_SPONSOR = org.technojays.first.jooq.tables.TeamSponsor.TEAM_SPONSOR;
}
